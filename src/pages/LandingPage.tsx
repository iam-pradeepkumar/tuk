...
const AutoplayVideo = ({ src }: { src: string }) => {
  ...
  useEffect(() => {
    if (videoRef.current) {
      // Force defaultMuted and muted to true to guarantee autoplay works in all browsers
      ...
      if (playPromise !== undefined) {
        playPromise
          ...
          .catch(err => {
            console.log("Autoplay paused initially, awaiting user click:", err);
          });
      }
    }
  }, [src]);
      ...
      if (isPlaying) {
        ...
        if (playPromise !== undefined) {
          playPromise
            ...
            .catch(err => console.log("Manual play interaction error:", err));
        }
      }
    }
  };
  ...
  return (
    <div className="relative w-full h-full group bg-black">
      ...
      {/* Control overlay in corner */}
      <div className="absolute bottom-4 right-4 flex items-center gap-2 z-20">
        {/* Play/Pause Button */}
        <button 
          onClick={togglePlay}
          className="bg-black/60 hover:bg-black/80 text-white p-2 rounded-full backdrop-blur-sm transition-all shadow-md cursor-pointer flex items-center justify-center w-8 h-8"
      ...
      <section id="party-video" className="py-12 md:py-20 bg-slate-50 border-b border-slate-200 flex justify-center text-center relative text-slate-800">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 w-full">
          ...
          <div className="relative aspect-video w-full rounded-2xl overflow-hidden shadow-2xl border-4 border-white bg-black">
            <div className="absolute inset-x-0 top-0 h-full overflow-hidden flex items-center justify-center">
              <AutoplayVideo src="/party-video.mp4" />
            </div>
          </div>
        </div>
      </section>
