...
import { X, Users, ShieldCheck, UserPlus } from 'lucide-react';
import { Wing, Officer, MemberApplication } from '../types';
...
export default function WingDetailsModal({ 
  ...
  useEffect(() => {
    ...
    const unsub = dbService.subscribeToCollection('memberships', (data: MemberApplication[]) => {
      setMemberCount(data.filter(m => m.wingId === wingId).length);
    });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-xl flex flex-col max-h-[85vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto w-full">
          {/* Member count widget */}
          <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-brand-blue/10 flex items-center justify-center text-brand-blue">
                <Users className="w-5 h-5" />
              </div>
              <div>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">{lang === 'ta' ? 'மொத்த உறுப்பினர்கள்' : 'Total Members'}</p>
                <p className="text-xl font-black text-slate-900">{memberCount}</p>
              </div>
            </div>
            ...
            <button 
              ...
              className="px-4 py-2 bg-brand-gold text-brand-blue text-sm font-bold rounded-xl shadow-sm hover:shadow-md transition-all flex items-center gap-2 cursor-pointer"
            >
              <UserPlus className="w-4 h-4" /> {lang === 'ta' ? 'இணைய பதிவு செய்' : 'Join Wing'}
            </button>
          </div>

          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2 mb-4 flex items-center gap-2">
